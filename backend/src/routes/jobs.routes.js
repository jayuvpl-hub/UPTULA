...
const upload = makeUploader('applications');
...
router.post('/jobs/:id/apply', authenticate, upload.single('resume'), async (req, res, next) => {
  try {
    ...
    // req.file.key is the S3 object key (was `/uploads/applications/${req.file.filename}`).
    const resumePath = req.file ? req.file.key : null;
